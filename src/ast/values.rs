...
impl<'a> ValueType<'a> {
    ...
    /// Creates a new decimal value.
    #[cfg(feature = "bigdecimal")]
    pub(crate) const fn numeric(value: BigDecimal) -> Self {
        Self::Numeric(Some(value))
    }

    /// Creates a new float value.
    pub(crate) const fn float(value: f32) -> Self {
    ...
    /// Creates a new uuid value.
    #[cfg(feature = "uuid")]
    pub(crate) const fn uuid(value: Uuid) -> Self {
        Self::Uuid(Some(value))
    }

    /// Creates a new datetime value.
    pub(crate) const fn datetime(value: DateTime<Utc>) -> Self {
        Self::DateTime(Some(value))
    }

    /// Creates a new date value.
    pub(crate) const fn date(value: NaiveDate) -> Self {
        Self::Date(Some(value))
    }

    /// Creates a new time value.
    pub(crate) const fn time(value: NaiveTime) -> Self {
        Self::Time(Some(value))
    }

    /// Creates a new JSON value.

    pub(crate) const fn json(value: serde_json::Value) -> Self {
    ...
    /// Returns a char if the value is a char, otherwise `None`.
    pub(crate) const fn as_char(&self) -> Option<char> {
        match self {
            Self::Char(c) => *c,
            _ => None,
        }
    ...
    /// Returns an `i32` if the value is a 32-bit signed integer, otherwise `None`.
    pub(crate) const fn as_i32(&self) -> Option<i32> {
        match self {
            Self::Int32(i) => *i,
            _ => None,
        }
    ...
    /// Otherwise `None`.
    #[cfg(feature = "bigdecimal")]
    pub(crate) const fn as_numeric(&self) -> Option<&BigDecimal> {
        match self {
    ...
    /// `true` if the `Value` is of UUID type.
    #[cfg(feature = "uuid")]
    pub(crate) const fn is_uuid(&self) -> bool {
        matches!(self, Self::Uuid(_))
    }

    /// Returns an UUID if the value is of UUID type, otherwise `None`.
    #[cfg(feature = "uuid")]
    pub(crate) const fn as_uuid(&self) -> Option<Uuid> {
        match self {
            Self::Uuid(u) => *u,
            _ => None,
        }
    ...
    /// `true` if the `Value` is a DateTime.
    pub(crate) const fn is_datetime(&self) -> bool {
        matches!(self, Self::DateTime(_))
    }

    /// Returns a `DateTime` if the value is a `DateTime`, otherwise `None`.
    pub(crate) const fn as_datetime(&self) -> Option<DateTime<Utc>> {
        match self {
            Self::DateTime(dt) => *dt,
            _ => None,
        }
    ...
    /// Returns a `NaiveDate` if the value is a `Date`, otherwise `None`.
    pub(crate) const fn as_date(&self) -> Option<NaiveDate> {
        match self {
            Self::Date(dt) => *dt,
            _ => None,
        }
    ...
    /// `true` if the `Value` is a `Time`.
    pub(crate) const fn is_time(&self) -> bool {
        matches!(self, Self::Time(_))
    }

    /// Returns a `NaiveTime` if the value is a `Time`, otherwise `None`.
    pub(crate) const fn as_time(&self) -> Option<NaiveTime> {
        match self {
            Self::Time(time) => *time,
            _ => None,
        }
    ...
    pub(crate) const fn as_json(&self) -> Option<&serde_json::Value> {
        match self {
